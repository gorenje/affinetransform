...
function CGContextAddArcCanvas(aContext, x, y, radius, startAngle, endAngle, anticlockwise) {
  aContext.arc(x, y, radius, startAngle, endAngle, anticlockwise);
}
function CGContextAddArcToPointCanvas(aContext, x1, y1, x2, y2, radius) {
...
function CGContextClosePathCanvas(aContext) { aContext.closePath(); }
function CGContextMoveToPointCanvas(aContext, x, y) { aContext.moveTo(x, y); }

function CGContextAddRectCanvas(aContext, aRect) {
  aContext.rect(CGRectGetMinX(aRect), CGRectGetMinY(aRect), 
...
function CGContextClipCanvas(aContext) {
  aContext.clip();
}
...
function CGContextAddArc(aContext, x, y, radius, startAngle, endAngle, clockwise)
{
    ...
    CGContextAddArcCanvas(aContext, x, y, radius, startAngle, endAngle, !clockwise);
}

function CGContextAddArcToPoint(aContext, x1, y1, x2, y2, radius)
{
    CGContextAddArcToPointCanvas(aContext, x1, y1, x2, y2, radius);
}

function CGContextAddCurveToPoint(aContext, cp1x, cp1y, cp2x, cp2y, x, y)
{
    CGContextAddCurveToPointCanvas(aContext, cp1x, cp1y, cp2x, cp2y, x, y);
}

function CGContextAddLineToPoint(aContext, x, y)
{
    CGContextAddLineToPointCanvas(aContext, x, y);
}
    ...
    for (; i < count; ++i)
    {
        ...
        switch (type)
        {
            case kCGPathElementMoveToPoint:         CGContextMoveToPointCanvas(aContext, element.x, element.y);
                                                    break;
            case kCGPathElementAddLineToPoint:      CGContextAddLineToPointCanvas(aContext, element.x, element.y);
                                                    break;
            case kCGPathElementAddQuadCurveToPoint: CGContextAddQuadCurveToPointCanvas(aContext, element.cpx, element.cpy, element.x, element.y);
                                                    break;
            case kCGPathElementAddCurveToPoint:     CGContextAddCurveToPointCanvas(aContext, element.cp1x, element.cp1y, element.cp2x, element.cp2y, element.x, element.y);
                                                    break;
            case kCGPathElementCloseSubpath:        CGContextClosePathCanvas(aContext);
                                                    break;
            case kCGPathElementAddArc:              CGContextAddArcCanvas(aContext, element.x, element.y, element.radius, element.startAngle, element.endAngle, element.clockwise);
                                                    break;
            case kCGPathElementAddArcTo:            //CGContextAddArcToPointCanvas(aContext, element.cp1x, element.cp1.y, element.cp2.x, element.cp2y, element.radius);
                                                    break;
        }
    }
}
...
{
    CGContextAddRectCanvas(aContext, aRect);
}
...
function CGContextAddRects(aContext, rects, count)
{
    ...
    {
        var rect = rects[i];
        CGContextAddRectCanvas(aContext, rect);
    }
}

function CGContextBeginPath(aContext)
{
    CGContextBeginPathCanvas(aContext);
}

function CGContextClosePath(aContext)
{
    CGContextClosePathCanvas(aContext);
}

function CGContextMoveToPoint(aContext, x, y)
{
    CGContextMoveToPointCanvas(aContext, x, y);
}
...
{
    CGContextFillRectCanvas(aContext, aRect);
}
...
function CGContextFillRects(aContext, rects, count)
{
    ...
    {
        var rect = rects[i];
        CGContextFillRectCanvas(aContext, rect);
    }
}
...
{
  CPLogConsole("Applying the transform to yhe canvase" );
  CPLogConsole("  a : " + anAffineTransform.a);
  CPLogConsole("  b : " + anAffineTransform.b);
  CPLogConsole("  c : " + anAffineTransform.c);
